...
def tabella(url_id, n, integrate):
    ...
    w2v_score, w2v_rank, = integrate.ms_w2v_key(url_id, n)            # w2v score, rank and n of keywords used
    print datetime.now(), "computing doc2vec most similar"
    d2v_score, d2v_rank, = integrate.ms_d2v(url_id, n)                # d2v score, rank and n of token used
    print datetime.now(), "computing tfidf most similar"
    tfidf_score, tfidf_rank = integrate.ms_tfidf(url_id, n)        # tfidf score, rank, and n of token used
    print datetime.now()
    print
    ...
    for i in range(0, n):                                           # create the rows for the table
        table.add_row([w2v_score[i], w2v_rank[i],
                       d2v_score[i], d2v_rank[i],
                       tfidf_score[i], tfidf_rank[i]])
...
def main():
    ...
    integrate = Integration(corpus, tfidf, index, tfidf_dict, tfidf_web, mean_dict, ball_tree, w2v_model, d2v_model)
